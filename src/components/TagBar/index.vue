<template>
  <div class="tag-bar-container">
    ...
    <div class="tag-bar-list">
      <div class="tag-bar-item" v-for="item of tagList" :key="`title-${item.id}`"
           :class="curTagId === item.id ? 'select-item' : ''">
        <img class="tag-bar-item-icon" @click="selectTag(item.id)" :src="item.icon" alt="">
        <div class="tag-item-item-title" @click="selectTag(item.id)">{{ item.name }}</div>
        <div class="tag-item-close">
          <i class="iconfont icon-close tag-bar-item-icon" @click="closeTag(item.id)"></i>
        </div>
      </div>
    </div>
  </div>
